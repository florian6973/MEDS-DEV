...
def diag_predicate(name: str, codes: list[str], fmt: str) -> str:
    """Render a diagnosis predicate that is robust to ICD vocab-version skew.

    The MEDS code space can contain granular ICD children that an OMOP-resolved (older-vocab) code
    list lacks -- e.g. ``ICD10CM/I21.40`` was added in the 2024 ICD-10-CM vocabulary, so a code list
    resolved against a 2023 OMOP build never produces it even though MEDS emits it. ICD codes are
    therefore matched by a ROOTED-PREFIX regex: each ICD code with no shorter ICD code as its prefix
    becomes a ``^prefix`` alternative, which captures that code AND all its present/future children
    without over-matching siblings outside the set (e.g. ``^ICD10CM/I21`` covers every ``I21.x``,
    including ``I21.40``, but not ``I22``/``I25``). Non-ICD codes (SNOMED, etc.) have no string
    hierarchy and stay exact. Dots are written ``[.]`` to avoid YAML/regex backslash escaping.
    """
    icd_pfx = ("DIAGNOSIS//ICD//",) if fmt == "mimic" else ("ICD10CM/", "ICD9CM/")
    icd = sorted(c for c in codes if c.startswith(icd_pfx))
    other = sorted(c for c in codes if not c.startswith(icd_pfx))
    roots = [c for c in icd if not any(c != p and c.startswith(p) for p in icd)]
    rx = "^(" + "|".join(r.replace(".", "[.]") for r in roots) + ")"
    if roots and other:
        return (f'  {name}_icd:\n    code: {{ regex: "{rx}" }}\n'
                f'  {name}_other:\n    code:\n      any:\n{code_block(other, indent="        ")}\n'
                f'  {name}:\n    expr: or({name}_icd, {name}_other)')
    if roots:  # ICD-only (e.g. mimic)
        return f'  {name}:\n    code: {{ regex: "{rx}" }}'
    return f'  {name}:\n    code:\n      any:\n{code_block(other, indent="        ")}'  # no ICD codes
...
def build_dataset_predicates(ami_codes: list[str], risk_codes: list[str], fmt: str) -> str:
    ...
    The outcome ``ami`` uses :func:`diag_predicate` (vocab-skew-robust ICD regex); ``risk_entry``
    stays an exact list (it only governs over-broad eligibility, where granular misses don't hurt).
