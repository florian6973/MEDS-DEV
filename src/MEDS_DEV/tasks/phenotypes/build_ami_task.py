...
def build_dataset_predicates(ami_codes: list[str], risk_codes: list[str], fmt: str) -> str:
    """Render just the `ami`/`risk_entry` predicates (for a dataset predicates.yaml override)."""
    return f"""\
  # AMI phenotype (tasks/phenotypes/ami.yaml) -- {fmt}-format concept-set overrides.
